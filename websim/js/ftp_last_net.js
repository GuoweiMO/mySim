...
function ftp_last_Init()
{
    $("#display10").empty();
    sigma.parsers.json('data/map_data_4.json', {
            container: 'display10',
            settings: {
        ...
        function (sig) {
            ...
            var myObject = {};
            $.get("data/fixed_toll_pricing_a", function (file) {
            ...
            sig.bind('clickNode', function (eve) {
                var nodeId = eve.data.node.id;
                var toKeep = sig.graph.neighbors(nodeId);
                toKeep[nodeId] = eve.data.node;
